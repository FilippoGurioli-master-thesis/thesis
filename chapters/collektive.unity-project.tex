...
This chapter faces the core research project produced for this thesis: a simulator for 3D \ac{CAS}.
...
The decision to adopt Unity as the platform for this study is driven by its effective balance between simulation fidelity, architectural flexibility, and development speed. Unlike higher-fidelity engines that may impose significant tooling overhead, Unity provides a streamlined workflow for agent-based modeling and accurate spatial constraints. For \ac{AC}, Unity is particularly attractive as a high-fidelity environment layer; it provides the realistic geometry and sensor interfaces while the aggregate runtime handles the collective logic and neighbor interactions.
...
\section{Implications for \acl{AC} Simulations}
...
The project goal is to bridge the Unity game engine with the \ac{AC} library named Collektive.
...
\begin{itemize}
	\item The system should maintain stable frame rate ( > 30 FPS) with at least 100 active collektive nodes in a low budget laptop (ryzen 7 5700U, 16GB DDR4, integrated GPU).
