...
The frontend is the last part that connects all point together into the final big picture. Since Unity follows the \ac{ECS} pattern, components must be exploited to achieve the goal of creating a simulation. As mentioned in the architecture, there is a central component that can be seen as the counterpart of the Collektive \monospace{Engine} named \monospace{SimulationManager}.

This component is attached exactly to one object in the scene. This component is responsible of configuring unity physics engine (and the unity environment in general) with configuration that enable reproducible simulations. This component is also the proxy for the Collektive \acp{API}, proxing the \monospace{Engine} \acp{API} to the Unity world. It has a reference to a static class that exploits the \monospace{P/Invoke} \cite{msft_pinvoke_2024} mechanism to interact with the Collektive native library. In the diagram \ref{fig:comm_channel} it is exposed the entire communication pipeline from Collektive domain to Unity.

\begin{diagram}{comm_channel}{Communication channel from the Collektive boundary down to the Unity domain.}
    \node [block, text width=20em] (engine) {
        {\centering \textbf{<<interface>>} \\ \textbf{Engine} \par}
        \vspace{-1ex}
        \rule{\linewidth}{0.4pt} \\
    ...
    \node [block, text width=12em, below of=engine, node distance=3cm] (nativekt) {
        {\centering \textbf{Native Kotlin APIs} \par}
    };

    \node [block, text width=10em, left of=nativekt, xshift=-4cm, yshift=-2cm] (proto) {
        {\centering \textbf{Proto Buffers} \\ \textbf{Shared Structures} \par}
    };

    \node [block, text width=12em, below of=nativekt, node distance=4cm] (nativecs) {
        {\centering \textbf{P/Invoke Import APIs} \par}
    };

    \node [block, text width=18em, below of=nativecs, node distance=4cm, xshift=-4cm] (simman) {
    ...
    \node [block, text width=9em, right of=simman, xshift=5cm] (mb) {
        {\centering \textbf{<<abstract>>} \\ \textbf{MonoBehaviour} \par}
    };

    \draw [arrow] (engine) -- node[right] {Exposes} (nativekt);
    \draw [arrow] (nativecs) -- node[right] {FFI Call} (nativekt);
    \draw [arrow] (nativekt) -- node[left] {Return Bytes} (nativecs);
    \draw [dashed] (proto.north) |- node[above, pos=0.7] {Generates .kt} (nativekt.west);
    \draw [dashed] (proto.south) |- node[below, pos=0.7] {Generates .cs} (nativecs.west);
    \draw [arrow] (simman) -- node[right] {P/Invoke} (nativecs);
